...
class CombinationAnalyzer(models.Model):
    ...
    def display_analysis_report(self):
        ...
        report_lines.append("TRANSFER GANGING COMBINATION ANALYSIS REPORT")
        report_lines.append("=" * 80)
        report_lines.append(f"Sheet Dimensions: {analysis['sheet_dimensions']}")
        report_lines.append("")
        ...
        report_lines.append(f"Total Single-Size Combinations: {stats['total_single_combinations']}")
        report_lines.append(f"Total Mixed-Size Combinations: {stats['total_mixed_combinations']}")
        report_lines.append(f"Highest Single Utilization: {stats['highest_single_utilization']}%")
        report_lines.append(f"Highest Mixed Utilization: {stats['highest_mixed_utilization']}%")
        report_lines.append(f"Average Single Utilization: {stats['average_single_utilization']}%")
        report_lines.append(f"Average Mixed Utilization: {stats['average_mixed_utilization']}%")
        report_lines.append(f"Combinations >90% Utilization: {stats['combinations_over_90_percent']}")
        report_lines.append(f"Combinations >95% Utilization: {stats['combinations_over_95_percent']}")
        report_lines.append("")
        
        # Single-Size Combinations
        report_lines.append("SINGLE-SIZE COMBINATIONS")
        report_lines.append("-" * 80)
        report_lines.append(f"{'Size':<12} {'Dimensions':<15} {'Max Qty':<8} {'Layout':<10} {'Utilization':<12} {'Waste Area'}")
        report_lines.append("-" * 80)
        
        for combo in analysis['single_size_combinations']:
            report_lines.append(
                f"{combo['size'].upper():<12} {combo['dimensions']:<15} "
        ...
        report_lines.append("TOP MIXED-SIZE COMBINATIONS (>90% Utilization)")
        report_lines.append("-" * 80)
        report_lines.append(f"{'Combination':<35} {'Items':<6} {'Utilization':<12} {'Efficiency':<10} {'Waste Area'}")
        report_lines.append("-" * 80)
        
        top_mixed = [c for c in analysis['mixed_size_combinations'] if c['utilization_percent'] >= 90][:20]
        for combo in top_mixed:
            report_lines.append(
                f"{combo['description']:<35} {combo['total_items']:<6} "
                f"{combo['utilization_percent']:>6.1f}%     {combo['layout_efficiency']:<10.1f} "
                f"{combo['waste_area']:>8.0f}mm²"
            )
        
        report_lines.append("")
        report_lines.append("HIGH-EFFICIENCY MIXED COMBINATIONS (80-90% Utilization)")
        report_lines.append("-" * 80)
        
        mid_mixed = [c for c in analysis['mixed_size_combinations'] 
                    if 80 <= c['utilization_percent'] < 90][:15]
        for combo in mid_mixed:
            report_lines.append(
                f"{combo['description']:<35} {combo['total_items']:<6} "
                f"{combo['utilization_percent']:>6.1f}%     {combo['layout_efficiency']:<10.1f} "
                f"{combo['waste_area']:>8.0f}mm²"
        ...
        # Log the report
        full_report = "\n".join(report_lines)
        _logger.info(f"Combination Analysis Report:\n{full_report}")
        ...
        return {
            ...
            'params': {
                ...
                'message': f"Generated analysis of {stats['total_single_combinations']} single and {stats['total_mixed_combinations']} mixed combinations. Check server logs for detailed report.",
                'type': 'success'
            }
        }